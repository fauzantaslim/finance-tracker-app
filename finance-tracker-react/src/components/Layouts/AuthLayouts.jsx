...
export default function AuthLayout({ children, titlePage, isLoginPage }) {
  return (
    <div
      className={
        "relative flex justify-center items-center min-h-screen bg-slate-100 p-4"
      }
    >
      <div className="w-full max-w-md bg-white border-2 border-black p-6 shadow-[8px_8px_0_0_rgba(0,0,0,1)] mb-2">
        {/* Logo SVG dengan warna yang sudah diperbaiki */}
        <div className="flex flex-col items-center mb-6">
          ...
          >
            {/* Background kuning sesuai brand OIKONO */}
...
174 -34 -1 -81 -8 -106 -17z"
        ...
        <h1 className="text-4xl font-extrabold mb-2 text-slate-900 tracking-tight">
          {titlePage}
        </h1>
        <p className="font-semibold text-slate-600 mb-6">Welcome!!</p>
        {children}
        <p className="mt-6 text-center">
          {isLoginPage ? "belum punya akun ?" : "sudah punya akun ?"}{" "}
          {isLoginPage && (
            <Link className="underline font-bold text-slate-900" to="/register">
              buat disini
            </Link>
          )}
          {!isLoginPage && (
            <Link className="underline font-bold text-slate-900" to="/">
              login disini
            </Link>
          )}
        </p>
      </div>
      <footer className="absolute bottom-0 left-0 right-0 text-center text-sm text-slate-500 bg-slate-100 mb-2">
        © {new Date().getFullYear()} Finance Tracker App
      </footer>
    </div>
  );
}
