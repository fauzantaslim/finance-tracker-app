...
export default function Sidebar() {
  ...
  // Use auth user data if available, otherwise fallback to default
  const user = {
    name: authUser?.full_name || "User",
    email: authUser?.email || "user@example.com",
  };
  
  // If no user is loaded but we have a token, fetch user data
  useEffect(() => {
    ...
    fetchUserData();
  }, [accessToken, authUser, dispatch, router]);
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Even if the API call fails, clear auth state and redirect
      dispatch(clearAuth());
      router.push("/login");
    } finally {
            ...
            {menuItems.map((item, index) => (
              ...
              >
                <Motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="flex items-center w-full px-4 py-3 rounded-lg hover:bg-yellow-100 font-semibold text-black transition border-2 border-transparent hover:border-black"
                  onClick={() => {
